...
class ChallanWithold(models.Model):
	...
	def generate_report(self):
		with xlsxwriter.Workbook("/home/nayyab/odoo10/projects/champion_new/customer_invoices.xlsx") as workbook:
			...
			worksheet.write('A1', 'Payment Section',main_heading)
			worksheet.write('B1', 'TaxPayer NTN',main_heading)
			worksheet.write('C1', 'TaxPayer CNIC',main_heading)
			worksheet.write('D1', 'TaxPayer Name',main_heading)
			worksheet.write('E1', 'TaxPayer City',main_heading)
			worksheet.write('F1', 'TaxPayer Address',main_heading)
			worksheet.write('G1', 'TaxPayer Status',main_heading)
			worksheet.write('H1', 'TaxPayer Bussiness',main_heading)
			...
			for x in self.challan_id:
				worksheet.write_string (row, col,x.tax_name,main_data)
...
class ChallanWitholdTree(models.Model):
	...
	def unlink(self):
		...
		withold = self.env['tax.withold'].search([('id','=',self.withold.id)])
		withold.state = 'unpaid'
